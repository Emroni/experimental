...
const bloomPass = new UnrealBloomPass(new THREE.Vector2(SIZE, SIZE), 1, 0, 0);
...
export default {
    ...
    size: SIZE,
    onTick: (time: number) => {
        lightIn.intensity = 4 * Math.sin(PI2 * 2 * time) + 4;
        lightOut.intensity = -2 * Math.sin(PI2 * 2 * time) + 2;

        container.rotation.x = 0.1 * Math.sin(PI2 * 2 * time) + 0.3;
        container.rotation.y = -PI2 * time;

        shape.rotation.x = 2 * container.rotation.y;
        shape.rotation.y = 2 * container.rotation.y;
        shape.scale.setScalar(0.1 * Math.abs(Math.sin(PI2 * time)) + 0.7);

        animate.forEach(cube => {
